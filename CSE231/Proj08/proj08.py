...
def getDictFormattedDatabases(listDatabases):
    dictDatabases = dict()
    for index in listDatabases:
    #for each language
        i = listDatabases.index(index)
        #set a variable for the index number
        for DBindex in range(len(listDatabases[i][1])):
            #for each word in the DB
            if listDatabases[i][1][DBindex] not in dictDatabases:
                #if the word is not already in the DB, then create a key and add it to the DB
                new_key = listDatabases[i][1][DBindex][0]
                dictDatabases[new_key] = listDatabases[listDatabases.index(index)][1][DBindex][1]
        listDatabases[i] = (listDatabases[i][0], copy.deepcopy(dictDatabases))
        dictDatabases.clear()
...
#split the text along whitespace
        if word in dictBasedDb[1]:

            transword = dictBasedDb[1][word]
...
I simply call the translation to replace each word in the text with the translated word, based upon language.
"""
def performDictBasedTranslations(preprocessedText, dictBasedDatabases):
    translatedText = ""    
    for language in dictBasedDatabases:
        langdex = dictBasedDatabases.index(language)
        translated = translateFromDict(preprocessedText, dictBasedDatabases[langdex])
        translatedText = translatedText + "-------\n" + dictBasedDatabases[langdex][0] + "\n-------\n" + translated + "\n"
